...
def getScore(living: float, material_attack: float, material_defense: float, magic_attack: float, magic_defense: float):
    """计算资质得分"""
...
if __name__ == "__main__":
    ...
    # 计算各项值
    living = math.ceil(20 - round((living_upper - living_curr), 2) / living_unit)
    material_attack = math.ceil(20 - round((material_attack_upper - material_attack_curr), 2) / material_attack_unit)
    magic_attack = math.ceil(20 - round((magic_attack_upper - magic_attack_curr), 2) / magic_attack_unit)
    magic_defense = math.ceil(20 - round((magic_defense_upper - magic_defense_curr), 2) / magic_defense_unit)
    material_defense = math.ceil(20 - round((material_defense_upper - material_defense_curr), 2) / material_defense_unit)
    
    print(f"当前数值：生命{living} 物攻{material_attack} 法攻{magic_attack} 法防{magic_defense} 物防{material_defense}")
    ...
    for combo in valid_combinations:
        ...
        # 计算新分数
        new_score = getScore(new_living, new_material_attack, new_material_defense, new_magic_attack, new_magic_defense)
        if new_score >= 90:
            # 构建输出字符串，只显示非零的果子数量
            output = []
            if l_add > 0:
    ...
    if results:
        ...
        for result in results:
            print(f"  总果子数: {result['total_used']}  {result['output']}  新得分: {result['new_score']}")
    else:
        print("\n无法在 6 个资质果内达到 90 分！")